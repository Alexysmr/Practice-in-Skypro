...
def reading_csv(file_name_csv: str) -> str:
    ...
    if file_name_csv != '':
        ...
        return 'Работа функции завершена с ошибкой: Имя файла данных отсутствует'  # для теста
    if os.path.exists(file_csv) and os.stat(file_csv).st_size != 0:
        try:
            ...
            return f'Работа функции завершена с ошибкой: {ex}'  # для теста
    else:
    ...
    return 'Работа функции завершена.'  # для теста, все равно возвращается None.
